...
const storeToken = async({client_id,client_secret}:{client_id:string,client_secret:string}):Promise<string|false>=>{
    ...
    if((!localStorage.getItem(LOCALSTORAGE_KEYS.cbApiToken) || parseInt(JSON.parse(localStorage.getItem(LOCALSTORAGE_KEYS.cbApiToken) as string).expire_time_inSeconds) < new Date().getTime()/1000)){
